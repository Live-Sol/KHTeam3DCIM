...
PROFILE_IMAGE VARCHAR2(200),          -- 프로필 이미지 경로 (추가됨)
IS_DELETED    NUMBER(1)     DEFAULT 0 NOT NULL, -- 탈퇴 여부 (0:활성, 1:탈퇴) (추가됨)

    CONSTRAINT PK_MEMBER PRIMARY KEY (MEMBER_ID),
    CONSTRAINT CK_DC_MEMBER_IS_DELETED CHECK (IS_DELETED IN (0, 1)) -- 탈퇴 여부 체크 제약조건
);
...
REQ_ID        NUMBER        NOT NULL, -- 신청서 번호
    ...
    REJECT_REASON VARCHAR2(300),          -- 반려 사유 (추가됨)
...
DEVICE_ID     NUMBER        NOT NULL, -- 장비 ID
    ...
    POWER_WATT    NUMBER(5)     DEFAULT 0, -- 전력 소비량 (추가됨)
    EMS_STATUS    VARCHAR2(10)  DEFAULT 'OFF', -- EMS 감시 모드 여부 (추가됨)
...
-- 관리자 계정 생성 (선택 사항 - 필요시 주석 해제 후 사용)
-- 비밀번호는 암호화되지 않은 상태이므로, 실제 운영 시에는 암호화된 값으로 넣어야 합니다.
-- INSERT INTO DC_MEMBER (MEMBER_ID, PASSWORD, NAME, EMAIL, ROLE)
...
-- 입고 이력 숨김 칼럼 추가 251224
ALTER TABLE DC_REQUEST ADD is_hidden NUMBER(1) DEFAULT 0 NOT NULL;

-- 논리 삭제 타이머 251226
ALTER TABLE DC_DEVICE ADD (DELETED_AT TIMESTAMP, DELETE_REASON VARCHAR2(255));
ALTER TABLE request ADD COLUMN delete_reason VARCHAR(255);